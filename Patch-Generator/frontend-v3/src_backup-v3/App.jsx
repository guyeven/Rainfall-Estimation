...
export default function App() {
  ...
  // Load detection defaults from backend
  useEffect(() => {
    let cancelled = false;
    const loadDefaults = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setDetectionParams({ ...FALLBACK_PARAMS, ...data });
        }
      } catch (err) {
        if (!cancelled) {
          setParamsError("Using built‑in defaults (failed to load server defaults).");
  ...
  const handleDetectionParamsChange = (next) => {
  ...
  const handleSaveDetectionDefaults = async () => {
    try {
      ...
      // best-effort; UI will still work with local state
      console.error("Failed to save detection defaults", err);
    }
  };
  ...
  const anyError = filesError || detectError || patchGeoError || patchImageError || paramsError;
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "right", fontSize: "0.8rem" }}>
          ...
          {lastTimestamp && (
            <div>Latest patch time: {formatTimestamp(lastTimestamp)}</div>
          )}
        </div>
      </header>
