...
# ---- Coefficient tables, from ITU-R P.838-3, Tables 1–4 ----
# Page 2 of the Recommendation. :contentReference[oaicite:1]{index=1}
...
def k_alpha_H(f_ghz: float) -> Tuple[float, float]:
    """
    k_H(f), alpha_H(f) for horizontal polarization (linear).

    f_ghz : frequency in GHz
    Returns:
        k_H (dB/km at R=1 mm/h), alpha_H (dimensionless)
    """
...
def k_alpha_V(f_ghz: float) -> Tuple[float, float]:
    """
    k_V(f), alpha_V(f) for vertical polarization (linear).
    """
    logf = math.log10(f_ghz)
...
def k_alpha(f_ghz: float, pol: Pol, elevation_deg: float = 0.0) -> Tuple[float, float]:
    """
    k(f), alpha(f) for given polarization and path elevation using
    ITU-R P.838-3 eqs. (4)–(5). :contentReference[oaicite:2]{index=2}

    For this app:
      - horizontal: τ = 0°
      - vertical:   τ = 90°
      - circular:   τ = 45°
    elevation_deg is θ (path elevation). We use θ = 0° by default.
    """
...
def gamma_specific(f_ghz: float, R_mm_per_h: float, pol: Pol) -> float:
    """
    Specific attenuation gamma_R (dB/km) at frequency f and rain rate R.

    gamma_R = k(f) * R**alpha(f)   (Eq. (1))
    """
    if R_mm_per_h <= 0:
        return 0.0
