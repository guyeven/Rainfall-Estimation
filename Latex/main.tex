...
The nonlinear optimizer used in this note is L-BFGS-B, a limited-memory quasi-Newton method for smooth constrained optimization. In the present setting, the optimization variable is the rainfall value at every pixel of the patch, so the problem dimension is the number of pixels in the patch. Rather than storing a full Hessian matrix, L-BFGS-B builds a low-memory approximation to local curvature information from the most recent iterates and gradients, and uses that approximation to propose a descent direction. A line search is then carried out along that direction, and the process is repeated until the stopping criteria on iteration count, function reduction, or projected gradient norm are met.

The suffix ``-B'' indicates that the method handles box constraints. Here those constraints enforce nonnegativity of the rainfall field, so every pixel estimate is required to remain in $[0,\infty)$. In practice, this means that the optimizer can exploit gradient-based curvature information while still respecting the physically natural requirement that rainfall rates should not become negative. The optimizer is initialized from the ILDW estimate, and the cost-reduction plots in the previous section illustrate how the weighted objective decreases from that starting point over successive iterations.

\section*{IDW and ILDW Pseudocode}
For reference, the no-comments pseudocode for the IDW and ILDW baselines is included below.

\includepdf[pages=-,pagecommand={\thispagestyle{plain}}]{idw_ildw_pseudocode_no_comments.pdf}
