...
\begin{algorithm}[H]
    \caption{Discretized IDW. Each link is replaced with a virtual gauge located at the midpoint of the link. The rain rate at the gauge is set to the uniform rain rate that reproduces the input link attenuation under ITU-R P.838-3. IDW is then applied with respect to these virtual gauges.}
    \DontPrintSemicolon
...
\begin{algorithm}[H]
    \caption{Discretized ILDW. Each link is treated as a line segment rather than a midpoint-only virtual gauge.}
    \DontPrintSemicolon
    \KwData{link set $\mathcal{L}$ and patch with set of pixels $\mathcal{P}$}
    \KwResult{rainfall field $\hat R$}
    ...
    \ForEach{pixel $p \in \mathcal{P}$}{
        ...
        \eIf{$M_p \neq \emptyset$}{
            ...
            $N_p \gets \{\ell \in \mathcal{L} : \mathrm{dist}(p.\mathrm{center}, \ell) \le r_{\max}\}$\;
            \eIf{$N_p = \emptyset$}{
                $\hat R_p \gets 0$\;
            }{
                $\hat R_p \gets \sum_{\ell \in N_p}\dfrac{\mathrm{dist}(p.\mathrm{center}, \ell)^{-2}}{\sum_{\ell' \in N_p}\mathrm{dist}(p.\mathrm{center}, \ell)^{-2}} \cdot \ell.\mathrm{rainrate}$\;
            }
        }
    }
\end{algorithm}
...
\]
and the Pearson correlation coefficient. To make the random-variable interpretation explicit, let $U$ be a pixel sampled uniformly at random from $P$, i.e., $U\sim\mathrm{Unif}(P)$, and define
\[
X_{P,s}\triangleq \widehat{R}_{P,s}(U),\qquad
Y_P\triangleq R_P(U).
\]
Then
\[
r_{P,s}\triangleq \operatorname{corr}(X_{P,s},Y_P)
=\frac{\operatorname{Cov}(X_{P,s},Y_P)}
{\sqrt{\operatorname{Var}(X_{P,s})}\,\sqrt{\operatorname{Var}(Y_P)}}.
\]
Under this uniform-pixel sampling view, these are exactly the empirical patch moments:
\[
\operatorname{Cov}(X_{P,s},Y_P)
=\frac{1}{|P|}\sum_{p\in P}
\bigl(\widehat{R}_{P,s}(p)-\mu_{X_{P,s}}\bigr)
\bigl(R_P(p)-\mu_{Y_P}\bigr),
\]
\[
\operatorname{Var}(X_{P,s})
=\frac{1}{|P|}\sum_{p\in P}
\bigl(\widehat{R}_{P,s}(p)-\mu_{X_{P,s}}\bigr)^2,
\quad
\operatorname{Var}(Y_P)
=\frac{1}{|P|}\sum_{p\in P}
\bigl(R_P(p)-\mu_{Y_P}\bigr)^2,
\]
with
\[
\mu_{X_{P,s}}\triangleq \mathbb{E}[X_{P,s}]
=\frac{1}{|P|}\sum_{p\in P}\widehat{R}_{P,s}(p),
\quad
\mu_{Y_P}\triangleq \mathbb{E}[Y_P]
=\frac{1}{|P|}\sum_{p\in P}R_P(p).
\]
...
Together, the attenuation-based analyses show that the nonlinear optimizer most consistently matches the measured link attenuations under the shared forward model.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Nonlinear optimizer}
  \end{subfigure}
  \caption{Case study for a rain event observed on January 31, 2023 at 09:00. The map on the left identifies the selected patch in geographic context, and the three solver panels show the same rainy-pixel decomposition as above for IDW, ILDW, and the nonlinear optimizer: $R_P$, $\widehat{R}_{P,s}$, $(R_P-\widehat{R}_{P,s})/R_P$, and $|R_P-\widehat{R}_{P,s}|/R_P$.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Nonlinear optimizer}
  \end{subfigure}
  \caption{Case study for a rain event observed on January 31, 2023 at 16:00. The map on the left identifies the patch location and footprint, and the three solver panels show the same rainy-pixel decomposition as above for IDW, ILDW, and the nonlinear optimizer: $R_P$, $\widehat{R}_{P,s}$, $(R_P-\widehat{R}_{P,s})/R_P$, and $|R_P-\widehat{R}_{P,s}|/R_P$. Together with the previous two figures, this case gives a compact qualitative comparison of solver behavior across distinct rainfall scenes.}
