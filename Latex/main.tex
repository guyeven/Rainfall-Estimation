...
\]
If one or more links cross the pixel, the estimate is taken to be the mean of the corresponding $R_\ell$ values. Otherwise, the estimate is a weighted average over nearby links within radius $r_{\max}$, with weights proportional to the inverse squared segment distance. We considered this baseline important because it preserves the same simple interpolation philosophy as IDW while respecting more of the measurement geometry of the links themselves.
...
For a patch $P$, let $x\in\mathbb{R}_{\ge 0}^{|\Omega(P)|}$ denote the unknown rainfall field written as a vector over pixels. The optimizer minimizes a weighted sum of four terms,
\[
J(x)\triangleq
\alpha_{\mathrm{atten}} J_{\mathrm{atten}}(x)
\;+\;
\alpha_{1d} J_{1d}(x)
\;+\;
\alpha_{2d} J_{2d}(x)
\;+\;
\alpha_{\mathrm{total}} J_{\mathrm{total}}(x).
\]
...
\alpha_{2d}=\frac{1}{2\,J_{2d}^{\mathrm{ILDW}}},\qquad
\alpha_{\mathrm{total}}=\frac{1}{J_{\mathrm{total}}^{\mathrm{ILDW}}}.
\]
More concretely, $J_{\mathrm{atten}}$ is the data-fidelity term:
\[
J_{\mathrm{atten}}(x)=\frac{1}{|\mathcal{L}(P)|}\sum_{\ell\in\mathcal{L}(P)}
\frac{\bigl(\widehat{A}_\ell(x)-A_\ell^{\mathrm{obs}}\bigr)^2}{L_\ell},
\]
where $\widehat{A}_\ell(x)$ is the attenuation predicted by the forward model from the rainfall field $x$, $A_\ell^{\mathrm{obs}}$ is the observed attenuation of link $\ell$, and $L_\ell$ is the link length in km. Thus $J_{\mathrm{atten}}$ measures how far the forward-model-implied attenuations are from the observed attenuations, with a normalization by link length and by the number of links in the patch.

The term $J_{1d}$ is a first-difference penalty:
\[
J_{1d}(x)=\frac{1}{|\Omega(P)|}\sum_{(u,v)\in\mathcal{E}(P)}\bigl(x_u-x_v\bigr)^2,
\]
...
\[
J_{2d}(x)=\frac{1}{|\Omega(P)|}\sum_{(p,q,r)\in\mathcal{T}(P)}
\bigl((x_q-x_p)-(x_r-x_q)\bigr)^2,
\]
...
\[
J_{\mathrm{total}}(x)=\frac{1}{|\Omega(P)|}\sum_{p\in\Omega(P)}x_p^2.
\]
...
For a patch $P$, let $\Omega(P)$ denote its pixel set, let $R_P : \Omega(P) \to \mathbb{R}_{\ge 0}$ be the ground-truth rainfall field, and let $\widehat{R}_{P,s} : \Omega(P) \to \mathbb{R}_{\ge 0}$ be the rainfall field predicted by solver $s$. We define a pixel $p$ to be rainy if $R_P(p) \ge 0.6$ mm/h, and we write
\[
\Omega_{\mathrm{rain}}(P)=\{p \in \Omega(P) : R_P(p)\ge 0.6\}.
\]
...
\Omega_{\mathrm{rain}}(P),\ d_3(p)\in B_i
\bigr\}\;.
\]
...
\operatorname{median}\bigl\{\mathrm{AE}_{P,s}(p): p\in \Omega_{\mathrm{nonrain}}(P),\ d_3(p)\in B_i\bigr\},
\]
...
\[
\alpha_{\mathrm{atten}} J_{\mathrm{atten}} + \alpha_{1d} J_{1d} + \alpha_{2d} J_{2d} + \alpha_{\mathrm{total}} J_{\mathrm{total}},
\]
...
\begin{figure}[H]
  ...
  \caption{Optimizer trace for the January 28th, 2023 case study. The plot shows the reduction of the weighted objective $J_{\mathrm{weighted\ sum}}$ and of the weighted constituent terms over L-BFGS-B iterations.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Optimizer trace for the January 31st, 2023 case study at 02:00. As above, the plot reports the iteration-by-iteration decrease of the weighted objective and its weighted constituent terms.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Optimizer trace for the January 31st, 2023 case study at 16:00. The weighted objective and weighted component terms are shown over the L-BFGS-B iterations for the same patch displayed in the spatial case-study panels above.}
\end{figure}
