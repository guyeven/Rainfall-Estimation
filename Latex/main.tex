...
Accurate rainfall estimation matters because it sits upstream of many decisions, including flood forecasting, water-resources management, agriculture, short-term nowcasting, and post-event analysis. In this thesis, rainfall estimation means reconstructing a spatial rainfall field from indirect observations, in particular from the attenuation measured on commercial microwave links. Inverse-distance weighting (IDW) and related interpolation rules are widely used as simple baselines for rainfall-field estimation from microwave-link data. Their appeal is practical: they are fast, easy to implement, and often produce visually plausible maps. However, these methods do not explicitly search for a rainfall field whose forward-model-implied link attenuations match the observed link attenuations. This thesis investigates whether a nonlinear optimization model can improve on IDW and on link-aware ILDW when the goal is not only to predict rainfall, but also to remain faithful to the measurements that generated the estimate.

\paragraph*{Contribution.} This thesis makes one main contribution: it shows that the nonlinear optimization model outperforms IDW and ILDW in both fitting observed link attenuations and reducing rainfall estimation error, as presented through attenuation-error summaries and distance-binned rainfall-error summaries.

\textcolor{green}{[Note: Consider restructuring this introduction following this structure: (i) goal of the thesis, (ii) contribution(s), and (iii) thesis structure, as in the style used in other master's theses.]}

% \paragraph*{Research questions.} We evaluate two questions. First, can rainfall estimation posed as a nonlinear optimization problem outperform interpolation-based baselines in regions that are weakly constrained by nearby links? Second, can the resulting rainfall field explain link attenuations more faithfully, that is, yield smaller mismatches between predicted and observed attenuations under the same forward model? These questions are examined later through distance-binned rainfall-error summaries, attenuation-error summaries, and objective-based analyses.
...
\paragraph*{Key findings at a glance.} Across the benchmark, the nonlinear optimizer gives the highest overall agreement with the ground-truth rainfall map and the best attenuation consistency. % ILDW consistently outperforms IDW, especially in patches where link placement and orientation make interpolation harder, while keeping the same interpolation-style computational simplicity.  
When we look at distance bins, all methods degrade as nearby-link coverage becomes weaker. The nonlinear optimizer remains clearly better than IDW in the low-coverage bins, whereas ILDW is competitive with IDW but not uniformly better in the farthest bins.

\subsection{Patch-Level Metrics}
We now turn to the empirical comparison of the three methods on the hundred-patch benchmark. We begin with whole-patch summaries, then examine how errors vary with link proximity, and finally report attenuation-consistency analyses.
...
This thesis compared three reconstruction strategies---IDW, ILDW, and a nonlinear optimization model---on a 100-patch benchmark built from OPERA rainfall fields and simulated microwave-link attenuations. Across the reported experiments, the nonlinear optimizer achieved the best attenuation consistency and the strongest overall agreement with the ground-truth rainfall maps. Its advantage was especially clear in distance bins corresponding to pixels that are less constrained by nearby links. ILDW remained a useful intermediate baseline: it preserves the simplicity of interpolation while making better use of link geometry than midpoint IDW.
...
The five case studies below pair a geographic view of the selected patch with the corresponding rainy-pixel solver outputs.

These five cases provide illustrative examples for qualitative comparison across different rain events.
...
\begin{figure}[H]
  ...
  \caption{Case study for a rain event observed on January 28, 2023 at 09:00. The combined panel shows the geographic map of the selected patch, the ground-truth rainfall field $R_P$, and, for each solver, the corresponding estimate $\widehat{R}_{P,s}$ together with the rainy-pixel relative error $(\widehat{R}_{P,s}-R_P)/R_P$.}
...
\begin{figure}[H]
  ...
  \caption{Case study for a rain event observed on January 31, 2023 at 09:00. The combined panel shows the geographic map of the selected patch, the ground-truth rainfall field $R_P$, and, for each solver, the corresponding estimate $\widehat{R}_{P,s}$ together with the rainy-pixel relative error $(\widehat{R}_{P,s}-R_P)/R_P$.}
...
\begin{figure}[H]
  ...
  \caption{Case study for a rain event observed on January 28, 2023 at 21:00. The combined panel shows the geographic map of the selected patch, the ground-truth rainfall field $R_P$, and, for each solver, the corresponding estimate $\widehat{R}_{P,s}$ together with the rainy-pixel relative error $(\widehat{R}_{P,s}-R_P)/R_P$.}
...
\begin{figure}[H]
  ...
  \caption{Case study for a rain event observed on January 31, 2023 at 02:00. The combined panel shows the geographic map of the selected patch, the ground-truth rainfall field $R_P$, and, for each solver, the corresponding estimate $\widehat{R}_{P,s}$ together with the rainy-pixel relative error $(\widehat{R}_{P,s}-R_P)/R_P$.}
...
\begin{figure}[H]
  ...
  \caption{Case study for a rain event observed on January 31, 2023 at 16:00. The combined panel shows the geographic map of the selected patch, the ground-truth rainfall field $R_P$, and, for each solver, the corresponding estimate $\widehat{R}_{P,s}$ together with the rainy-pixel relative error $(\widehat{R}_{P,s}-R_P)/R_P$.}
