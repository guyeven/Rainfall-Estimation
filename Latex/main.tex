...
The runtime results in \Cref{tab:runtime_summary} provide an initial indication of the computational feasibility of such an application. On the benchmark hardware, Convex Solver reconstructed a patch in an average of $76.03$ seconds (median $71.31$ seconds; range $32.76$--$153.12$ seconds). This suggests that the solver runtime may be compatible with near-real-time reconstruction at the benchmark scale, although operational latency would also include collecting, delivering, and preprocessing the CML measurements.
