...
\titleformat{name=\section}
  ...
  {\chapteropeningtopspace\Huge Chapter \thesection}
  {7.0ex}
  {\Huge}
...
For each patch in the benchmark list, we first extract the corresponding rainfall crop from the EURADCLIM map. We then refine that crop from the native \SI{2}{km} grid to a finer \SI{125}{m} grid by splitting each coarse EURADCLIM pixel into smaller subpixels and assigning each subpixel the parent pixel's rainfall value. We then smooth the refined field with a Gaussian filter whose standard deviation is one refined pixel (\SI{125}{m}), so that nearby pixels receive greater weight than more distant pixels. The filter uses a $7\times7$-pixel neighborhood, extending up to three refined pixels (\SI{375}{m}) in each grid direction, and repeats the nearest boundary value where this neighborhood extends beyond the patch. Because the Gaussian weights are normalized to sum to one, this smoothing redistributes rainfall among neighboring pixels rather than systematically scaling it, although exact preservation of total rainfall is not guaranteed at the patch boundaries, where nearest-value padding can count boundary values multiple times. This smoothing is a pragmatic preprocessing step: it softens the artificial block boundaries created by subdividing each coarse pixel, but it does not recover information about how rainfall actually varies within each original \SI{2}{km} pixel. The resulting high-resolution rainfall field serves as ground truth for both simulating link attenuations and evaluating the solvers.

The microwave-link geometry is not geographically co-located with the selected EURADCLIM patch locations. Instead, we use records from the 4TU-NL dataset \citep{overeem2023cml}, which contains observations and metadata from an operational CML network covering the Netherlands. We use the recorded endpoint geometry and frequencies as a realistic network template, place this network in the coordinate system of each selected patch, and retain only links whose endpoints fall inside the patch. Thus, the link geometries and frequencies are derived from actual Dutch CML infrastructure, whereas their placement relative to the EURADCLIM rainfall fields is synthetic. The attenuation of each retained link is then simulated from the radar-derived rainfall field rather than taken from the original operational CML measurements. Across the 100 patches, this procedure retains between 939 and 1996 links per patch (mean 1324).
...
{\color{black}In this benchmark, $A_\ell^{\mathrm{obs}}$ is generated from the known ground-truth rainfall field using the forward model described above. Except for the diagnostic Solver(GT), however, the reconstruction methods receive only these attenuation observations and the link metadata, not the ground-truth rainfall field itself. In an operational setting, $A_\ell^{\mathrm{obs}}$ would instead be estimated from actual CML signal measurements after dry-baseline estimation and correction or filtering of non-rain effects. Thus, the attenuation-misfit term does not require radar ground truth; the synthetic attenuations used here serve as controlled substitutes for measurements that would be obtained from the CML network.}
...
We evaluate each solver in four ways: overall agreement with the ground-truth rainfall map, how errors change as distance from links increases, wet/dry classification behavior, and agreement with observed link attenuations under the forward model. We first report patch-level summaries across all 100 benchmark patches, and then provide qualitative case studies in the appendix. We additionally report end-to-end runtime for IDW, ILDW, and Convex Solver to provide an initial indication of their computational cost. All optimization-based results reported below use the same objective terms and weights described above, with a maximum budget of 2000 optimizer iterations per patch.
...
{\color{black}An important next step is to repeat the comparison of reconstruction methods using measurements from an operational CML network. In the current benchmark, the observed attenuations are generated by applying the same forward model used by the solvers to the radar-derived ground-truth rainfall fields, producing clean and model-consistent observations. In a real-time application, the attenuation-mismatch term would instead be driven by rain-induced attenuations estimated from current CML signal measurements, while independent radar or rain-gauge observations would be used only as reference data for evaluation. Such an experiment is needed to determine whether the improvements over IDW persist in the presence of measurement noise, changing link availability, wet-antenna attenuation, baseline drift, and mismatch between the forward model and real signal behavior.}

The runtime results in \Cref{tab:runtime_summary} provide an initial indication of the computational feasibility of such an application. On the benchmark hardware, Convex Solver reconstructed a patch in an average of $76.03$ seconds (median $71.31$ seconds; range $32.76$--$153.12$ seconds). This suggests that the solver runtime may be compatible with near-real-time reconstruction at the benchmark scale, although operational latency would also include signal acquisition, preprocessing, and data transfer.
