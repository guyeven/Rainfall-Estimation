...
In this thesis, we introduce a nonlinear optimization formulation for rainfall reconstruction and compare it against interpolation-based baselines. More specifically, we ask whether this nonlinear formulation can improve on the interpolation-based baselines by reducing rainfall-reconstruction error, especially in regions that are far from nearby links, while also matching the observed link attenuations more faithfully.

To test this question, we construct a 100-patch benchmark from radar-derived rainfall fields. We then simulate microwave-link attenuations with a physical model relating rainfall to link attenuation and compare three reconstruction strategies: IDW, a variant of IDW that we call ILDW, and a nonlinear optimization method.

The remainder of the thesis reviews the background and prior work, presents the benchmark and methods, reports the empirical comparison, and closes with a discussion of the main conclusions and limitations.
