...
def aggregate_distance_bin_mean_std(
    ...
    """
    For each distance bin label, compute across patches:

      - avg_mean[bin] = mean over patches of the per-patch mean_abs for that bin
      - std_means[bin] = standard deviation over patches of the per-patch mean_abs for that bin

    (This is the "std of patch means" variant you asked for.)

    Skips rows with n_pixels == 0 or non-finite mean_abs values.
    """
    ...
    avg_mean = np.array(
        [np.mean(means_per_bin[lab]) if len(means_per_bin[lab]) else np.nan for lab in dist_labels],
        dtype=np.float64,
    )
    std_means = np.array(
        [np.std(means_per_bin[lab], ddof=0) if len(means_per_bin[lab]) else np.nan for lab in dist_labels],
        dtype=np.float64,
    )
    return avg_mean, std_means
...
def main():
    ...
    idw_rmax_m = float(deep_get(cfg, "idw.r_max_m", 15000.0))
    ...
    # Compute profiles for rainy (relative abs error) and nonrainy (absolute diff)
    mean_sol_rainy, std_sol_rainy = aggregate_distance_bin_mean_std(distance_rows, dist_labels, "rainy")
    mean_idw_rainy, std_idw_rainy = aggregate_distance_bin_mean_std(distance_rows_gt_idw, dist_labels, "rainy")

    mean_sol_non, std_sol_non = aggregate_distance_bin_mean_std(distance_rows, dist_labels, "nonrainy")
    mean_idw_non, std_idw_non = aggregate_distance_bin_mean_std(distance_rows_gt_idw, dist_labels, "nonrainy")

    plot_distance_profiles(
        dist_labels=dist_labels,
        mean_sol=mean_sol_rainy,
        std_sol=std_sol_rainy,
        mean_idw=mean_idw_rainy,
        std_idw=std_idw_rainy,
        gt_label=gt_label_plot,
        sol_label=sol_label_plot,
        idw_label=idw_label_plot,
        mask_type="rainy",
        out_png=img_dir / "distance_profile_rainy_GTvsSOL_vs_GTvsIDW.png",
    )
    plot_distance_profiles(
        dist_labels=dist_labels,
        mean_sol=mean_sol_non,
        std_sol=std_sol_non,
        mean_idw=mean_idw_non,
        std_idw=std_idw_non,
        gt_label=gt_label_plot,
        sol_label=sol_label_plot,
        idw_label=idw_label_plot,
        mask_type="nonrainy",
        out_png=img_dir / "distance_profile_nonrainy_GTvsSOL_vs_GTvsIDW.png",
    )
