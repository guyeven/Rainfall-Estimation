...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if R0_from_IDW:
        print(f"[init] Using IDW initialisation: r_max_m={idw_r_max_m}, power={idw_power}, eps_m={idw_eps_m}, default={idw_default_value}")
    ...
    for i, est_path in enumerate(est_files, start=1):
        ...
        try:
            # Build kwargs so we can pass IDW-init options only if the solver supports them
            kwargs = dict(
                est_input_json=str(est_path),
                lam=float(lam),
                mu=float(mu),
                eps=eps,
                R0=R0,
            ...
            sig = inspect.signature(solver.solve_lbfgsb_and_save)
            params = sig.parameters

            # If your solve_rain_lbfgsb.py has been patched for IDW init, pass through:
            if "R0_from_IDW" in params:
                kwargs["R0_from_IDW"] = R0_from_IDW
            if "idw_r_max_m" in params:
                kwargs["idw_r_max_m"] = idw_r_max_m
            if "idw_power" in params:
                kwargs["idw_power"] = idw_power
            if "idw_eps_m" in params:
                kwargs["idw_eps_m"] = idw_eps_m
            if "idw_default_value" in params:
                kwargs["idw_default_value"] = idw_default_value

            solver.solve_lbfgsb_and_save(**kwargs)
