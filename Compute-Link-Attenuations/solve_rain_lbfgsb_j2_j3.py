...
def make_objective(prob: EstProblem, lam: float, mu: float, eps: float, sum_idw: float):
    """
    Objective:
      J = sum_{valid l} ((A_hat_l - A_obs_l)/L_l)^2
        + lam * sum_(p,q in N4) (Rp - Rq)^2
        + mu  * (sum_p Rp - sum_p IDW(p))^2
    """
    pix = prob.pix_idx
    li = prob.link_idx
    ds = prob.ds_km
    ...
    def f_and_g(R_flat: np.ndarray) -> Tuple[float, np.ndarray]:
        ...
        # --- smoothness term on R (no log) ---
        du = R[n_u] - R[n_v]
        J_smooth = float(np.dot(du, du))

        w = 2.0 * du
        g_smooth = np.bincount(n_u, weights=w, minlength=prob.P) - np.bincount(n_v, weights=w, minlength=prob.P)

        # --- sum-to-IDW term ---
        sum_r = float(np.sum(R))
        diff = sum_r - float(sum_idw)
        J_shrink = diff * diff
        g_shrink = np.full_like(R, 2.0 * diff)
...
def solve_lbfgsb_and_save(
    ...
    *,
    ...
    if idw_field_from_est_input is None:
        raise RuntimeError("idw_baseline.idw_field_from_est_input is required for J3.")
    ...
    sum_idw = float(np.sum(idw_field))

    fun, jac = make_objective(prob, lam=lam, mu=mu, eps=eps, sum_idw=sum_idw)
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Solve rainfall inverse problem with SciPy L-BFGS-B (J2 + J3). Supports YAML/JSON configs."
    )
