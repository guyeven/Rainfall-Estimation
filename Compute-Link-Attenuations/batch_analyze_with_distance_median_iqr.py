...
def plot_distance_median_iqr(
    *,
    ...
    def draw_series(x0: np.ndarray, p50: np.ndarray, p25: np.ndarray, p75: np.ndarray, label: str):
        for xi, m, lo, hi in zip(x0, p50, p25, p75):
            if not (np.isfinite(m) and np.isfinite(lo) and np.isfinite(hi)):
                continue
            ax.vlines(xi, lo, hi, linewidth=2)
            ax.hlines(lo, xi - cap, xi + cap, linestyles="dotted", linewidth=1.5)
            ax.hlines(hi, xi - cap, xi + cap, linestyles="dotted", linewidth=1.5)
            ax.plot([xi], [m], marker="o", markersize=5)
        # Dummy handle for legend
        ax.plot([], [], marker="o", linestyle="-", label=label)

    draw_series(x - off, p50_sol, p25_sol, p75_sol, f"{gt_label} vs {sol_label}")
    draw_series(x + off, p50_idw, p25_idw, p75_idw, f"{gt_label} vs {idw_label}")
